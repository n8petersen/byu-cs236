...
public:
    Scanner(const string &input, int lineNum) : input(input), lineNum(lineNum) { }
    ...
    int checkString(string str_check) {
        if (str_check.find('\'') == string::npos) { // if no ' is not found, return -1.
            return -1;
        }
        else return str_check.find('\''); // otherwise return its index
    }
    ...
    vector<Token> scanToken() {
        ...
        while(!input.empty()) {
            ...
            switch (input.at(0)) {
                ...
                case '\'': // "Let's eat" is invalid. "I''m coding" is valid. // TODO: Add a '' escape character, so we get 1 string instead of 2.
                ...
                case '#': // TODO: Add capability for multiline comments #| <comment> |#

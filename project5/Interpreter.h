...
public:
	...
	// 	do  // Fixed Point Algorithm to keep evaluating rules that depend on each other or themself.
	// 	{
	...
	void evaluateAllRules(Graph graph, queue<set<int>> sccForest)
	{
		// idea1
		// int sccForestSize = sccForest.size();
		// for (unsigned int i = 0; i < sccForestSize; i++)
		// {

		// 	// int count = 0;
		// 	int ruleID = 0;
		// 	// cout << "SCC:";
		// 	for (auto &currRule : sccForest.front())
		// 	{
		// 		ruleID = currRule;

		// 		// Add the logic here to evaluate the rules basic on their dependency conditions, and FPA.
		// 		// we also want to keep track of how many times we did the SCC before ending.
		// 		if (true)
		// 		{
		// 			cout << " R" << ruleID << endl;
		...
		// idea 2
		// for each scc in scc forest:
		// 		if size = 1, and scc does NOT depend on itself
		// 				evaluate rule once
		// 		else do FPA evaluation
		//

		int sccForestSize = sccForest.size();
		for (int i = 0; i < sccForestSize; i++) // for each scc in scc forest
		{
			cout << "SCC: ";
			int numPasses = 0;

			set<int> currentScc = sccForest.front(); // this scc from forest
			set<int>::iterator itr = currentScc.begin();
			map<int, Node> nodes = graph.getNodes();
			Node currentNode = nodes[*itr];
			set<int> edges = currentNode.getEdges();
			set<int>::iterator check = edges.find(*itr);
			bool found = false;
			if (check != edges.end())
			{
				found = true;
			...
			if (currentScc.size() == 1 && !found) // if size == 1 && the node doesn't depend on itself
			{
				cout << rules.at(*itr).toString() << "." << endl;
				evaluateRule(rules.at(*itr)); // evaluate once
				numPasses++;
			}
			else
			{ // do FPA to evaluate the same set of rules until no new facts are added
				int sizeBefore = 0;
				int sizeAfter = 0;
	...
	static pair<Graph, Graph> makeGraph(const vector<Rule> &rules)
	{
		...
		Graph fwdGraph(rules.size());
		Graph backGraph(rules.size());
		for (unsigned fromID = 0; fromID < rules.size(); fromID++)
		{
			...
			for (unsigned pred_idx = 0; pred_idx < fromRule.getBodySize(); pred_idx++)
			{
				...
				for (unsigned toID = 0; toID < rules.size(); toID++)
				{
					...
					{
						fwdGraph.addEdge(fromID, toID);
						backGraph.addEdge(toID, fromID);
					}
				}
			}
		}
		// pair<Graph, Graph> graphs = { fwdGraph, backGraph };
